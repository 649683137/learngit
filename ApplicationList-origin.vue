<template>
  <div v-loading="loading">
      ...
      <el-col :span="24">
        <sticky-top-bar :custom-style="{'justify-content':'space-between'}">
          <div class="z-left-part">
            <span>筛选:</span>
            <el-select size="small" style="width: 80px" v-model="enable" placeholder="是否可用" @change="loadData">
              <el-option
                label="全部"
                :value="-1">
              </el-option>
            ...
            <el-input v-model="keyword" size="small" style="width: 180px" placeholder="搜索 标题/公司"></el-input>
            <el-button @click="reset" size="mini">重置</el-button>
            <el-button @click="loadData()" size="mini">刷新</el-button>
          </div>

          <div class="z-right-part">
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div style="display: flex;align-items: center;">
                <img style="height:30px;width: 30px; flex-shrink:0; border:1px solid #404040;border-radius: 50%;"
    ...
    <el-dialog
      title="添加应用" :visible.sync="addApplicationVisible" width="350px"
      ...
      <el-form labelWidth="60px">
        ...
        <el-form-item label="公司">
          <!--          <el-input v-model.trim="applicationCompanyTmp"></el-input>-->
  ...
  export default {
    ...
    methods: {
      updateNumber (row, type) {
        ...
        this.axios.post(this.urls.application[type].update, p).then(() => {
          this.$message({
            type: 'success',
            message: '修改成功'
          })
      ...
      async searchCompany (qs, cb) {
        let res = await this.axios.get(this.urls.company.search, {params: {key: qs}})
        let companyList = []
        res.forEach(item => {
      ...
      updateCompany (aid) {
        ...
        this.axios.post(this.urls.application.company.update, param).then(res => {
          this.loadData()
      ...
      delApplication (aid) {
        this.$confirm('将删除当前应用库，是否继续?', '提示', {
          ...
          this.axios.post(this.urls.application.del, params).then(res => {
            this.loading = true
            this.$message({
              showClose: true,
              type: 'success',
      ...
      addApplication () {
        ...
        this.axios.post(this.urls.application.add, params).then(res => {
          this.loading = true
          this.$message({
            showClose: true,
            type: 'success',
      ...
      downloadPdf (apId, key) {
        ...
        this.axios.get(this.urls.application.pdf.download, {params: param}).then(res => {
//          alert(res)
//          window.location.href = res
      ...
      searchKey: debounce((vm) => {   // 包装请求列表函数，达到延时请求
        vm.loadData()
      }, 600),
      sizeChange (pageSize) {
...
//        alert(id + '-' + enable)
        ...
        this.axios.post(this.urls.application.enable, params).then(() => {
          this.$message({
            showClose: true,
            type: 'success',
            title: '修改可用性',
      ...
      loadData () {
        ...
        // application list
        this.axios.get(this.urls.application.list, {params: params}).then(res => {
    ...
    activated () {
      ...
      } else if (queryKey === '' && queryKey !== this.keyword) {  // 传入空串，强制刷新(避开 '' 为false 的判定)
        this.loading = true
        this.keyword = queryKey
      } else {
        this.loadData()
