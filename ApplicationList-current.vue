<template>
  <div v-loading="loading">
    ...
    <el-row>
      <el-col :span="24">
        <div
          style="margin: 10px 10px 0px 10px;display: flex;flex-flow:row wrap;align-items:center;"
        >
          <div
            style="width: 210px;border: 1px solid #e2d7d7;border-radius: 10px;line-height: 38px;text-align: center"
          >
    ...
    <el-row>
      <el-col :span="24">
        <el-table :data="applicationList">
          ...
          <el-table-column label="封面" align="center" width="100px">
            <template v-slot:default="scope">
              <div
                style="width:50px;height:50px;border:1px solid;border-radius:10px;margin:auto"
                :style="{background:'url('+scope.row.icon+') center center / contain no-repeat'}"
              ></div>
            </template>
          </el-table-column>
          <el-table-column prop="application_title" label="标题">
            <template v-slot:default="scope">
              <div style="display:flex">
                <img
                  v-if="scope.row.sokoo_recognization===1"
                  src="../../assets/img/logo_sokoo.jpg"
                  alt
                  title="sokoo 认证"
                  style="width:30px;height:30px;border:1px solid #404040;border-radius:50%;flex-shrink:0;"
                />
                ...
                <div style="flex-grow: 1; margin-left: 5px">
                  ...
                  >{{ scope.row.application_title }}</a>
                </div>
              </div>
            </template>
          </el-table-column>

          <el-table-column prop="application_company" label="公司">
            <template v-slot:default="scope">
              <span
                slot="reference"
                @click="scope.row.editCompanyTag=!scope.row.editCompanyTag"
                style="cursor: pointer;color: #1f2d3d;font-size:12px;font-weight: 600"
              >{{scope.row.application_company ? scope.row.application_company : '----'}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="page_count" label="页数">
            <template v-slot:default="scope">
              <el-badge
                :value="scope.row.page_count"
                type="primary"
          ...
          <el-table-column label="置顶" align="center" width="150px">
            <template v-slot:default="scope">
              <el-input-number
          ...
          <el-table-column label="操作" align="center" fixed="right" width="133">
            <template v-slot:default="scope">
              <el-tooltip class="item" content="带水印pdf" effect="dark" placement="top-start">
                <el-button
                  v-if="scope.row.has_watermark_pdf===1"
              ...
              <el-tooltip class="item" content="原版pdf" effect="dark" placement="top-start">
                <el-button
                  v-if="scope.row.has_origin_pdf===1"
                  title="下载"
                  type="success"
                  circle
                  size="mini"
    ...
    <el-dialog
      title="上传pdf"
      width="400px"
      :visible.sync="uploadApPdfVisible"
      :close-on-click-modal="false"
    >
      <form
        class="el-form upload-form el-form--label-top"
        ...
        <div class="el-form-item is-required">
          <div class="el-form-item__content">
            ...
            >
              ...
              <div v-if="!previewPdf" class="el-upload__text">
                将PDF拖到此处或
                <em>点击选择</em>
              </div>
            </div>
    ...
    >
      ...
      <el-row>
        <el-col :span="24">
          <el-teble :data="applicationList">
            <el-el-column prop="id" label="ID" align="center" width="70px"></el-el-column>
          </el-teble>
        </el-col>
      </el-row>
